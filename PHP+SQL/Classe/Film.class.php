...
class Film
{
  ...
  /**
   * Accesseur magique (getter) d'une propriété de l'objet
  ...
  /**
   * Mutateur magique (setter) qui exécute le mutateur de la propriété en paramètre 
   * @param string $prop, nom de la propriété
 ...
 /**
  ...
  public function setFilm_id($film_id) {
    unset($this->erreurs['id_film']);
    $regExp = '/^[1-9]\d*$/';
    if (!preg_match($regExp, $film_id)) {
      $this->erreurs['id_film'] = 'Numéro de film incorrect.';
    }
  ...
  public function setFilm_titre($film_titre) {
    ...
    if (!preg_match($regExp, $film_titre)) {
      $this->erreurs['titre'] = 'Au moins un caractère.';
    }
  ...
  public function setFilm_annee_sortie($film_annee_sortie) {
    unset($this->erreurs['annee_sortie']);
    if (!preg_match('/^\d+$/', $film_annee_sortie) ||
        $film_annee_sortie < self::ANNEE_PREMIER_FILM  || 
        $film_annee_sortie > date("Y")) {
      $this->erreurs['annee_sortie'] = "Entre ".self::ANNEE_PREMIER_FILM." et l'année en cours.";
    }
  ...
  public function setFilm_resume($film_resume) {
    unset($this->erreurs['resume']);
    $film_resume = trim($film_resume);
    $regExp = '/^\S+(\s+\S+){4,}$/';
    if (!preg_match($regExp, $film_resume)) {
      $this->erreurs['resume'] = 'Au moins 5 mots.';
    }
  ...
  public function setFilm_affiche($film_affiche) {
    unset($this->erreurs['affiche']);
    $film_affiche = trim($film_affiche);
    $regExp = '/^.+\.jpg$/';
    if (!preg_match($regExp, $film_affiche)) {
      $this->erreurs['affiche'] = "Vous devez téléverser un fichier de type jpg.";
    }
